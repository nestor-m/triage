	...
	<body>
		...
		<div id="show-paciente" class="content scaffold-show" role="main">
			...
			<ol class="property-list paciente">
				...
				<g:if test="${pacienteInstance?.persona}">
				<li class="fieldcontain">
					<span id="persona-label" class="property-label"><g:message code="paciente.persona.label" default="Persona" /></span>
					
						<span class="property-value" aria-labelledby="persona-label"><g:link controller="persona" action="show" id="${pacienteInstance?.persona?.id}">${pacienteInstance?.persona?.encodeAsHTML()}</g:link></span>
